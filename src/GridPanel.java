...
////////////////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////////////
        ...
        protected int[] getFieldNumberOffsets( int row,
                                               int column )
        {
            int barWidthIn = (AppConfig.getInstance( ).getBarGridBarWidth( ) - 1) / 2;
        ...
        protected void drawSeparators( Graphics gr )
        {
            int barWidth = AppConfig.getInstance( ).getBarGridBarWidth( );
    ...
    ////////////////////////////////////////////////////////////////////
        ...
        private void show( )
        {
            CrosswordView view = App.getInstance( ).getView( );
            if ( view != null )
...
////////////////////////////////////////////////////////////////////////
                       ...
                       int numRows )
    {
        ...
        cellSize = AppConfig.getInstance( ).getGridCellSize( Grid.Separator.BLOCK );
...
////////////////////////////////////////////////////////////////////////
    ...
    protected void paintComponent( Graphics gr )
    {
        ...
        if ( !isEditing( ) &&
             ((rect.x + rect.width - 1) / cellSize == column) &&
             ((rect.y + rect.height - 1) / cellSize == row) )
        {
            // Fill background
            ...
            if ( grid.isEntryValue( row, column ) )
            {
                ...
                x += (cellSize - charWidth) / 2;
                gr.drawString( Character.toString( ch ), x,
                               y + GuiUtilities.getBaselineOffset( cellSize, fontMetrics ) );
                x += charWidth;
            }
            else
                x += (cellSize - Caret.WIDTH) / 2;

            // Draw caret
            if ( caretDrawnPosition != null )
            {
                gr.setColor( config.getViewColour( CrosswordView.Colour.CARET ) );
                gr.fillRect( x, y + Caret.VERTICAL_MARGIN, Caret.WIDTH,
                             cellSize - 2 * Caret.VERTICAL_MARGIN - 1 );
            }
        }
        ...
        else
        {
            ...
            if ( isEditing( ) )
            {
                ...
                for ( Grid.IndexPair indices : isolatedCells )
                    gr.fillRect( indices.column * cellSize + 1, indices.row * cellSize + 1, cellSize - 1,
                                 cellSize - 1 );
                gr.setColor( config.getViewColour( CrosswordView.Colour.
                ...
                for ( row = 0; row < numRows; ++row )
                {
                    for ( column = 0; column < numColumns; ++column )
                    {
                        if ( grid.isIncorrectEntry( row, column ) )
                            gr.fillRect( column * cellSize + 1, row * cellSize + 1, cellSize - 1,
                                         cellSize - 1 );
                    }
                }
            }
            ...
            if ( isEditing( ) )
            {
                ...
                gr.setColor( config.getViewColour( isFocusOwner( )
                                                                ? CrosswordView.Colour.FOCUSED_EDITING_BOX
            ...
            else
            {
                ...
                for ( row = 0; row < numRows; ++row )
                {
                    for ( column = 0; column < numColumns; ++column )
                    {
                        if ( grid.isEntryValue( row, column ) )
                        {
                            ...
                            y = row * cellSize + 1 +
                                                    GuiUtilities.getBaselineOffset( cellSize, fontMetrics );
                ...
                if ( caretDrawnPosition != null )
                {
                    ...
                    x = column * cellSize + 1;
                    y = row * cellSize + 1;
                    int caretX = x + (cellSize - Caret.WIDTH) / 2;
                    if ( grid.isEntryValue( row, column ) )
                    {
                        int charWidth = fontMetrics.charWidth( grid.getEntryValue( row, column ) );
                        caretX = x + (cellSize - charWidth) / 2 + charWidth;
                    }
                    gr.clipRect( x, y, cellSize - 1, cellSize - 1 );
                    gr.setColor( config.getViewColour( CrosswordView.Colour.CARET ) );
                    gr.fillRect( caretX, y + Caret.VERTICAL_MARGIN, Caret.WIDTH,
                                 cellSize - 2 * Caret.VERTICAL_MARGIN - 1 );
                }
            }
        }
...
////////////////////////////////////////////////////////////////////////
    ...
    {
        setSelection( fields,
                      (fields.enabled && (fields.getField( ) != null))
                                                                    ? fields.getField( ).getStartIndices( )
                                                                    : null );
    }
    ...
    public void drawCaret( boolean draw )
    {
        ...
        if ( draw )
        {
            if ( caretPosition != null )
            {
                ...
                repaint( caretDrawnPosition.column * cellSize + 1, caretDrawnPosition.row * cellSize + 1,
                         cellSize - 1, cellSize - 1 );
            }
        }
    ...
    public Clue.FieldList incrementCaretColumn( int increment )
    {
        ...
        if ( caretPosition != null )
        {
            ...
            boolean navigateOverSeparators = AppConfig.getInstance( ).isNavigateOverGridSeparators( );
    ...
    {
        return ( selectedFields.isEmpty( ) ? Direction.NONE
                                           : selectedFields.getField( ).getDirection( ) );
    }
